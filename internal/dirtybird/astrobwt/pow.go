...
// Ported from derohe astrobwt/astrobwtv3 (DERO Foundation, see LICENSE-DERO.txt).
// Algorithm code below is byte-for-byte identical to the reference; only the
// scratch handling changed (caller-owned ScratchData instead of sync.Pool,
// no per-hash recover).
...
// multi-buffer path (sha256mb) is unavailable.
func sha256Fallback(p []byte) [32]byte { return sha256.Sum256(p) }

// astroBWTv3 computes the DERO AstroBWTv3 hash using caller-owned scratch.
...
// after it returns, scratch.sa[:data_len] holds the SA whose little-endian
// bytes are the final SHA-256 input. Split out so HashPair can batch two
// nonces' final hashes through the 2-way SHA-NI path.
func astroBWTv3Stream(input []byte, scratch *ScratchData) (data_len uint32) {
	...
	ts = stageLap(stagePrologue, ts)

	tries := uint64(0)

	// v114 template-marker bookkeeping (pure bookkeeping — never touches the
	// hash state). Semantics match the dirtybird C++ wolfCompute exactly,
	// including the tries>1 index quirk (see zig miner astrobwt.zig:283-306).
	templateIdx := 0
	chunkCount := uint32(1)
	firstChunk := uint32(0)
	...
	// flush the final template marker and record the count (C++ semantics:
	// chunkCount > 0 always holds here)
	scratch.markers[templateIdx] = uint16(firstChunk<<7 | chunkCount)
	scratch.nTemplates = uint32(templateIdx + 1)
	ts = stageLap(stageWolf, ts)
