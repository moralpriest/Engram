...
// ---- Camera2 QR Scanner C Bridge ----

void startCamera(JNIEnv* env) {
	if (start_qr_camera_method == 0) {
		LOG_FATAL("startCamera: method not available (Java not patched)");
		return;
	}
	(*env)->CallStaticVoidMethod(env, current_class, start_qr_camera_method);
}

void stopCamera(JNIEnv* env) {
	if (stop_qr_camera_method == 0) {
		LOG_FATAL("stopCamera: method not available (Java not patched)");
		return;
	}
